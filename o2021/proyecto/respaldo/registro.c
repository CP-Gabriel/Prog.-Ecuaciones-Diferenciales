/*
* @file: registro.c
* @brief: Esta documento guarda las funciones que se utilizan para los usuarios.c
...
/*
* @brief: Esta funciones permite hacer el registro de los datos personales del usuario.
* @authors: Gabriel Coronado/Segio Esquivel/ Sebastian Alanis.
* @date: 31/11/21
* @param usuarios[] es un struct que almacena los datos de los usuarios.
* @param x es un acomulador que se va sumando cada vez que ingresa un nuevo usuario.
* @return *nuevoUsuario es un apuntador que almacena 1 si los datos ingresados estan completos
*/
		...
		do
		{
			...
			*nuevoUsuario=1;
			...
			printf ("\n4.Ingrese su clave de lector: ");
                        ...
                        for (j=0; j<tam; j++)
                        {
                                if (((aux[i]<'0') && (aux[i]>'9'))&&((aux[j]<'a') || (aux[j]>'z')))
                                {
                                        error=1;
					printf (" \nerror: %i", error);
                                }
			...
			printf ("\n5.Ingrese su correo electrónico: ");
                        ...
                        for (j=0; j<tam; j++)
                        {
                                if ((aux[i]!='@')&&(aux[i]!='_')&&(aux[i]!='-')&&(aux[i]!='.')&&((aux[i]<'0') && (aux[i]>'9'))&&((aux[j]<'a') && (aux[j]>'z')))
                                {
                                        error=1;
					printf (" \nerror: %i", error);
                                }
...
/*
* @brief: Esta función permite guardar la direccion del usuario.
* @authors: Gabriel Coronado/Segio Esquivel/ Sebastian Alanis.
* @date: 31/11/21
* @param usuarios[] es un struct que almacena los datos de los usuarios.
* @param *nuevoUsuario es apuntador que almacenta 1 si los datos personales del usuario estan completos.
* @param *datos es un apuntador que almacena 1 si los la direccion del usuario esta completa.
* @param x es un acomulador que se va sumando cada vez que ingresa un nuevo usuario.
* @return *datos es un apuntador que almacena 1 si los la direccion del usuario esta completa..
*/
        ...
        for (i=x; i<(x+1); i++)
        {
			...
			*datos=1;
			...
			if (*nuevoUsuario==1)
			{
				*nuevoUsuario=0;
				printf ("\nTus datos personales están guardados.\n");
                        ...
                        for (j=0; j<tam; j++)
                        {
                                if ((aux[j]!=' ')&&((aux[i]<'0') && (aux[i]>'9'))&&((aux[j]<'a') && (aux[j]>'z')))
                                {
                                        error=1;
                                        printf (" \nerror: %i", error);
                                }
			...
			printf ("\n2.Ingrese su número interior: ");
                        ...
                        for (j=0; j<tam; j++)
                        {
                                if (((aux[i]<'0') && (aux[i]>'9'))&&((aux[i]<'a') && (aux[i]>'z')))
                                {
                                        error=1;
                                        printf (" \nerror: %i", error);
                                }
...
/*
* @brief: Esta función guarda los datos de los usuarios en un archivo de texto.
* @authors: Gabriel Coronado/Segio Esquivel/ Sebastian Alanis.
* @date: 31/11/21
* @param usuarios[] es un struct que almacena los datos de los usuarios.
...
* @param *datos es un apuntador que almacena 1 si los la direccion del usuario esta completa.
* @param x es un acomulador que se va sumando cada vez que ingresa un nuevo usuario.
* @param *registrado es un apuntador que se va sumando cada vez que se guardan los datos de un usuario.
* @return *registrado que es un apuntador que se va sumando cada vez que se guardan los datos de un usuario.
*/
