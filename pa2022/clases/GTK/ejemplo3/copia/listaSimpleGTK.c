...
* @brief Este programa permite manejar una lista Push/Pop desde una interfaz diseñada en GTK.
*	 Un usurio puede colocar una cadena en el campo de ingreso de datos y tiene como
*	 opciones dar Push o Pop. Si se oprime el botón de Push, la cadena del campo se anexa
*	 a la lista simple; si se oprime el botón de Pop, el programa libera de la lista el
*	 último elemento de la lista que fue ingresado. El programa manda imprimir un mensaje,
*	 a modo terminal, en el que indica cuál fue la operación realizada.
...
//void pushTexto(GtkWidget *boton, gpointer estructura, gpointer estatus);
...
////////////////////////////////////
	base.label2 = gtk_label_new("Estatus de la lista:");
//	base.estatus = gtk_entry_new();
	base.estatus = gtk_label_new("");
//	base.estatus = gtk_label_new(base);
...
////////////////////////////////////
//	gtk_entry_set_text(GTK_ENTRY(base.estatus), &base);
...
//void pushTexto(GtkWidget *boton, gpointer estructura, gpointer estatus)
{
	...
	gtk_label_set_text(GTK_LABEL(pt->estatus), datos);
	gtk_label_set_text(GTK_LABEL(pt->label2), "Se dio push a:");
//	gtk_entry_set_text(GTK_ENTRY(pt->estatus), datos);
...
void popTexto(GtkWidget *boton, gpointer estructura)
{
	char datos[20];
	ref *pt = (ref *)estructura;
//	strcpy(datos, pt->texto);
	strcpy(datos, gtk_entry_get_text(GTK_ENTRY(pt->entrada)));
	gtk_label_set_text(GTK_LABEL(pt->estatus), datos);
	gtk_entry_set_text(GTK_ENTRY(pt->entrada),"");
	gtk_label_set_text(GTK_LABEL(pt->label2), "Se dio pop a:");
	pt->contPop++;
